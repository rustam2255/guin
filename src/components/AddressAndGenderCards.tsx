...
function createCenterTextPlugin(getTotal: () => number): Plugin<"doughnut"> {
  return {
    id: "centerTextPlugin",
    afterDraw(chart) {
      const { ctx } = chart;
...
export function DoughnutCard({
  ...
  const centerTextPlugin = useMemo(() => {
    return createCenterTextPlugin(() => finalTotal);
  }, [finalTotal]);
  ...
  return (
    <div className="w-full rounded-2xl bg-white p-4 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button className="flex items-center gap-1 text-[13px] text-[#3B82F6] hover:underline">
          {buttonText}
      ...
      <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-[180px_1fr]">
        ...
        <div className="w-full">
          <div className="mb-2 grid grid-cols-[minmax(0,1fr)_90px_70px] items-center border-b border-gray-200 pb-2 text-[13px] font-medium text-gray-500">
            <p>{tableHeader}</p>
            <p className="text-right">Mahkumlar</p>
          ...
          <div className="space-y-3">
            {items.map((item, index) => {
              const percent =
                finalTotal > 0 ? Math.round((item.value / finalTotal) * 100) : 0;
